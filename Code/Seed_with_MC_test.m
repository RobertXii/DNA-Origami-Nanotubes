...
correction = 1;
SElength = 10*10^-9;
origami_radius = 30*10^-9;
N = 16 * 0.693 * (origami_radius)^2 / (correction * eff_alpha * (SElength)^2);%(5*10^-9)^2 = H;16 * 0.693  = 16ln2; 6.25 * 10^-16 = 25nm**2
disp(N)
dt = 1;
total_t = 3600*(initialTemp-finalTemp)/ramp+3600*14+3600*24*0;
disp(total_t/3600)
mu = 0.00089;
...
mol_m = 5 * 10^-9 * 3 ;%mol of seed
m_nm3 = mol_m * 0.1;%1nM = 10^-6 * 6*10^23/10^27 = 6*10^-10 nm^-3
m = mol_m * 6.02 * 10^23 * 10^3;% #per m^3
...
for r = 1:n+Additional_num
    if mod(o, npm) == 0 && initialTemp >= finalTemp %change the kernal every 1min
        initialTemp = initialTemp - dpm;
        M = 1 * initialTemp * 2 * 1.38 * 10^-23 * melt(initialTemp, H, S, m_nm3)^2 / (3 * mu);
        for j = 1:kinds
            for k = 1:kinds
                if mod(k+j, 3) == 0
                    const(j,k) = M*(j+k)^2 / (3*(k*j+N*(k+j)*(k*j)^3/(2*(k^3+j^3))));
                else
                    const(j,k) = M*(j+k)^2 / (3*(k*j+N*(k+j)*(k*j)^3/(1*(k^3+j^3))));
                end
            end
        end
        o = o+1;
        ...
        for i = 1:kinds 
            ...
            loop(t+1,i) = loop(t,i) + form_loop(1,i)*origamis(t,i)*dt - lose*loop(t,i)*dt; %loop incraesed
            dori(1,i) = dori(1,i) + add + lose*loop(t,i);
...
disp(all(end,3)/sum(all(end,:)));
disp(all(end,7)/sum(all(end,:)));
...
function alpha = find_alpha(k)
    ...
    alpha = 1*connect/(connect+not_connect);
end
%}
