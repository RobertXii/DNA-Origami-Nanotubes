...
finalTemp = 303;
eff_alpha = 0.0013;
N = 4.436 * 10^17 * 6.25 * 10^-16 / eff_alpha;
alpha = 0.0074;
beta = 0.0066;
dt = 1;
total_t = 172800/2;
...
for r = 1:n+(initialTemp-finalTemp)*60/ramp+10
    if mod(o, npm) == 0 && initialTemp - finalTemp >= 0.1
        ...
        for j = 1:7
            for k = 1:7
                ...
                if mod(k, 3) == 1 && mod(j, 3) == 1
                    const(j,k) = const(j,k)/3;
                else 
                    const(j,k) = const(j,k)*2/3;
                end
            end
        end
        o = o+1;
