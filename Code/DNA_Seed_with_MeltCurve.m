...
eff_alpha = 0.47;
correction = 1;
N = 16 * 0.693 * 6.25 * 10^-16 / (correction*eff_alpha * (5*10^-9)^2);
disp(N)
dt = 1;
total_t = 3600*(initialTemp-finalTemp)/ramp+3600*5+3600*0;
disp(total_t/3600)
mu = 0.00089;
H = -82.8*4184.0 / 8.3;
S = -222.6*4.184 / 8.3;
...
for r = 1:n+Additional_num
    if mod(o, npm) == 0 && initialTemp - finalTemp >= 0.1 %change the kernal every 1min
        ...
        for j = 1:kinds
            for k = 1:kinds
                if mod(k+j, 3) == 0
                    const(j,k) = M*(j+k)^2 / (3*(k*j+0.5*N*(k+j)*(k*j)^3/(k^3+j^3)));
                else
                    const(j,k) = M*(j+k)^2 / (3*(k*j+N*(k+j)*(k*j)^3/(k^3+j^3)));
                end
            end
        end
        o = o+1;
...
grid off;
legend('1mers', '2mers', '3Loop','3mers','4mers', '5mers', '6Loop','6mers', '7mers', '8mers','9Loop');
hold off;
%{
